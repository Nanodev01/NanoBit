...
import { ArrowLeft, Save, Upload, Link as LinkIcon, Sparkles, FolderGit2, Image as ImageIcon } from "lucide-react";
...
export default function NewProjectPage() {
  ...
  const [gifMode, setGifMode] = useState<"file" | "url">("file");
    ...
    <main className="p-8 max-w-3xl">
      <header className="mb-10">
        ...
        <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
          <FolderGit2 className="w-7 h-7 text-cyan-400" /> Nuevo Proyecto
        </h1>
        <p className="text-slate-500 font-mono text-sm">
          Registra una nueva aplicación, sube capturas, portadas o añade un GIF para reproducir al pasar el mouse.
        </p>
      </header>
      ...
      <form onSubmit={handleSubmit} className="bg-white/5 border border-white/10 p-6 sm:p-8 rounded-xl space-y-6">
        ...
        <div className="space-y-2">
          ...
          <textarea 
            ...
            required
            rows={4}
            placeholder="Describe la arquitectura, propósito y tecnologías clave..."
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-xs font-mono text-slate-400">URL Demo / Web en Producción (Opcional)</label>
            <input 
              type="url" 
              name="url"
              value={demoUrl}
            ...
            {demoUrl && (
              <p className="text-[11px] text-cyan-400/80 font-mono flex items-center gap-1">
                <Sparkles className="w-3 h-3" /> Auto-preview activado: si no subes imagen, se generará una captura en vivo de este link.
              </p>
            )}
          </div>
          <div className="space-y-2">
          ...
          {coverMode === "file" ? (
            <div className="p-4 border-2 border-dashed border-white/10 hover:border-cyan-500/40 rounded-xl bg-black/30 transition-colors">
              ...
              <p className="text-[11px] text-slate-500 font-mono mt-2">
                Formatos: JPG, PNG, WEBP, GIF. Imagen estática o miniatura principal.
              </p>
            </div>
          ) : (
            <input 
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-slate-500 font-mono">
                Se reproducirá automáticamente al pasar el cursor sobre la tarjeta en el portfolio.
              </p>
            </div>
            <div className="flex items-center gap-1 p-1 bg-black/50 border border-white/10 rounded-lg">
              <button
        ...
        <div className="flex justify-end pt-4">
          ...
          >
            {loading ? "Guardando proyecto..." : <><Save className="w-4 h-4" /> Guardar Proyecto</>}
          </button>
        </div>
      </form>
    </main>
